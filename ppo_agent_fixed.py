...
class ActorNetwork(nn.Module):
    ...
    def get_action(
        ...
        if deterministic:
            # Skip log_prob in deterministic mode (not needed for evaluation)
            log_prob = torch.zeros(action.shape[0] if len(action.shape) > 1 else 1)
        else:
            log_prob = dist.log_prob(
                raw if self.dist_type == "gaussian" else action
            )
